...
contract RovrFarm is Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable rovrToken;
    ...
    uint16 public vaultRewardRate = 2500; // 25% is sent to vault rewards
    ...
    // Update the given pool's Rovr allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function handleFee(uint256 _pid, uint256 _feeAmount) private {
        ...
        uint256 liquifyAmount = _feeAmount.mul(liquifyRate).div(10000);
        if (liquifyAmount > 0) {
            pool.lpToken.safeTransfer(feeAddress, liquifyAmount);
        }
        // Send 25% to vault rewards
        uint256 vaultRewardAmount = _feeAmount.sub(burnAmount).sub(
            liquifyAmount
        );
        ...
        if (returnedTokenAmount > 0) {
            rovrToken.safeTransfer(vaultAddress, returnedTokenAmount);
        }
        if (!success || returnedETHAmount == 0) return;
        swapETHToToken(address(rovrToken), returnedETHAmount, vaultAddress);
    }

    /**
     * @notice Remove lp which has ETH in pair
     */
    function removeLiquidityETH(
        address _lpToken,
        address _token,
        ...
        )
    {
        ...
        returns (uint256 swappedETHAmount) {
            success = true;
            ethAmount = swappedETHAmount;
            tokenAmount = IERC20(_token).balanceOf(address(this)).sub(
                balanceBefore
            );
        } catch (
    ...
    function swapETHToToken(
        ...
        uint256 _ethAmount,
        address _to
    ) private returns (uint256 tokenAmount, bool success) {
        ...
        {
            tokenAmount = IERC20(_token).balanceOf(_to).sub(balanceBefore);
            success = true;
        } catch (
            bytes memory /* lowLevelData */
        ) {
    ...
    // Deposit LP tokens to MasterChef for Rovr allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(block.number >= startBlock, "Can not deposit before start");
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(rovrToken)) {
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(rovrToken)) {
            totalRovrInPools = totalRovrInPools.sub(amount);
        }
    ...
    {
        if (
            _amount > 0 && rovrToken.balanceOf(address(this)) > totalRovrInPools
        ) {
            ...
            uint256 rovrBal = rovrToken.balanceOf(address(this)).sub(
                totalRovrInPools
            );
            if (_amount >= rovrBal) {
                _amount = rovrBal;
            ...
            if (directHarvestAmount > 0) {
                rovrToken.safeTransfer(_to, directHarvestAmount);
            }
    ...
    function setFeeAllocRates(
        uint16 _liquifyRate,
        uint16 _burnRate,
        uint16 _vaultRewardRate
    ) external onlyOwner {
        require(
            _liquifyRate + _burnRate + _vaultRewardRate == 10000,
            "Invalid rates"
        );
        liquifyRate = _liquifyRate;
        burnRate = _burnRate;
        vaultRewardRate = _vaultRewardRate;
    }
    ...
    function updateEmissionRate(uint256 _rovrPerBlock) public onlyOwner {
        massUpdatePools();
